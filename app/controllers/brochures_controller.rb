class BrochuresController < ApplicationController
  ...
  before_action :set_brochure, only: [:itemdetail, :create]
  ...
  def create
    ...
    if @brochure.owner_type == 'default'
      broch = Brochure.new
      broch.width = 17
      broch.height = 11
      ...
      broch.src = "/public/uploads/brochure/src/"+ broch.id.to_s + "/goodu.json"
    ...
    File.open(directory_name + "/goodu.json", 'wb') do |f|
      f.write(params[:image])
    end
    File.open("#{Rails.root}/public/uploads/svg.xml", 'wb') do |f|
       f.write(params[:svgimage])
    end
    @brochure = broch
  end
  ...
  def brochures_area_change
    ...
    if(params[:libtype])
      if(params[:libtype] == 'design')
        ...
        @pBrochures = totalBrochure.where(:owner_type => "user")
      end
    else
      @pBrochures = totalBrochure.where(:owner_type => "default")
    end
  ...
  def json_params
    params.permit(:image, :pdfimage, :svgimage, :fromsvgtopdf, :preimage)
  end
